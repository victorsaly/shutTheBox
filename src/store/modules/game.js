...
const state = {
    game : [],
    gameIsVisible: false,
    gameAllowSounds : true,
    gameLevel:1
};


const mutations = {
    'SET_GAME_LEVEL'(state, level){
        state.gameLevel = length;
    },
    'SET_GAME_ALLOW_SOUND'(state, bool){
        state.gameAllowSounds = bool;
    },
    'SET_GAME' (state, game){
        state.game = game;
    },
...
const actions = {
    toogleSound: ({commit, state}) => {
        commit('SET_GAME_ALLOW_SOUND', !state.gameAllowSounds);
    },
    ...
    restartGame:({commit, dispatch}, payload) => {
        if (payload){
            commit('RESTART_TILES');
        }
        //dispatch('shuffleTiles');
...
const getters = {
    ...
    gameAllowSounds: state => {
        return state.gameAllowSounds;
    },
    gameLevel: state =>{
